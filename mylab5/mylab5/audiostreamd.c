...
if (argc < 7) {
    ...
    printf("./audiostreamd 57666 58933 250 90 1 logs\n");
	exit(1);
 }
...
while(1) {
	...
	newSockFd = accept(sockFd,(struct sockaddr *) &ctaddr,&ctN);
	if(newSockFd <0){
	...
	{											// File exist
		...
		printf("client udp %s : portnumber %d\n",inet_ntoa(ctaddr.sin_addr),rcvUDPport );
		//exit(1);
/*
    	udpClientAddr.sin_family = AF_INET;
    	udpClientAddr.sin_addr.s_addr = htonl(inet_ntoa(ctaddr.sin_addr));
    	udpClientAddr.sin_port = htons(rcvUDPport);                                  // bind udpclientport to udp client
