...
*/
    ...
    ssize_t wlen = sendto(sockFd,buf,strlen(buf),0,(struct sockaddr *)&svaddr,sizeof(svaddr));
           // send msg to server
   ...
   bzero(rdbuff,blocksize);
    ...
    int filesz  =0;
    int svsize = sizeof(svaddr);
    
    //byterd = recvfrom(sockFd,rdbuff,sizeof(rdbuff),0,(struct sockaddr*)&svaddr, &svsize);
    while( 1)       // keep reading from server

    {
        byterd = recvfrom(sockFd,rdbuff,sizeof(rdbuff),0,(struct sockaddr*)&svaddr, &svsize);

        filesz += byterd;                                       // record file size
        printf("Bytes received from server :%d : content is %s, totalt is %d  \n",byterd,rdbuff,filesz);      
        fwrite(rdbuff, 1,byterd,fp);
        bzero(rdbuff,blocksize);
        if (strcmp(rdbuff,"EOF")==0)
        {
            printf("EOF\n");
            break;
        }
