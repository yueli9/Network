...
sockFd = socket(AF_INET,SOCK_DGRAM,0);                                 // set up socket
...
*/
  ...
  //int n = read(newSockFd,buf,sizeof(buf)-1);       // read msg from socket
  int n = recvfrom(sockFd, buf, sizeof(buf)-1, 0, (struct sockaddr *)&ctaddr,&ctN);
   ...
   buf[n] = '\0';
	...
	msg[l] =NULL;
  	...
  	if (k==0) {							             // child process
  		...
  		char filename[50];
    	sprintf(filename, "%s%s", "./tmp/", msg[0]); 
        ...
        while (1)					                                // start reading from file 
        {
            ...
            if(nr > 0)							                   // start sending bytes
            {
                ...
                int  sbsize = sendto(sockFd,rbuf,strlen(rbuf),0,(struct sockaddr *)&ctaddr,sizeof(ctaddr));
                if (sbsize<0)
                {
                  printf("ERROR sendback\n");
                }
                else{
                  totalsize += nr;
                  printf("success sendback %d, totalsize %d \n", sbsize,totalsize);
                }
            ...
            if (nr < blocksize && nr >=0)				       // check EOF
            {
                if (feof(fp))
                    ...
                    strcpy(rbuf, "EOF");
                    sendto(sockFd,rbuf,strlen(rbuf),0,(struct sockaddr *)&ctaddr,sizeof(ctaddr));
