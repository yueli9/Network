...
int main(int argc, char *argv[])
{
    ...
    printf("overlaybuild dst-IP dst-port routerk-IP ... router2-IP router1-IP overlay-port build-port\n");
    // $dst-IP$dst-port$routerk-IP$...$router2-IP$router1-IP$src-port=port$
    ...
    vpnPortNum = atoi(argv[argc -2]);                                // get vpn port number  
    ...
    svaddr.sin_addr.s_addr = inet_addr(argv[argc - 3]);
